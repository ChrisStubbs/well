param($uiTestFolder, $siteUrl)

#$uiTestFolder = "..\src\4. Test\Well.Test.UI\"
#$siteUrl = "http://ho-ms-qa1/well/dashboard/"

cls
function RunProtractorTests
{
	$fileExe = $uiTestFolder + "node_modules\.bin\protractor"
    $confPath = $uiTestFolder + "conf.js"
	$result = & $fileExe $confPath --params.baseUrl=$siteUrl
	$code = $LASTEXITCODE
	$exitC = 1
	if ($code -eq 0 -or $code -eq 110 -or $code -eq 120 -or $code -eq 210 -or $code -eq 430)
	{
		$exitC = 0
	}
    $result > ..\ProtractorOutput.txt
	Write-Host $result
	return $exitC
}
$exitCode = RunProtractorTests

Write-Host "Exiting with code $exitCode"

exit $exitCode